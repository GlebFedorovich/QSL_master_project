...
class MySpinlessMixed(MixedXKModel):
    ...
    def init_terms(self, model_params):
        ...
        if model_params["flux"] == "pi_third":
            for k in range(Ly):
                
                '''on-site coupling'''
                self.add_onsite([(-t) * 2 * np.cos(2 * np.pi/Ly *k + 2 * np.pi/3), (-t) * 2 * np.cos(2 * np.pi/Ly *k - 2 * np.pi/3), (-t) * 2 * np.cos(2 * np.pi/Ly *k)], k, 'N')

                ''' hopping between rings'''
                strength = [(-t) * (-1 + (1) * np.exp(-1j * 2 * np.pi/Ly * k)), (-t) * (np.exp(-1j * np.pi/3) + (1) * np.exp(-1j * 2 * np.pi/Ly * k)), (-t) * (np.exp(+1j * np.pi/3) + (+1) * np.exp(-1j * 2 * np.pi/Ly * k))]
                dx = 1
                self.add_coupling(strength, k, 'Cd', k, 'C', dx, op_string='JW', plus_hc=True)
        
        if model_params["flux"] == "No":
            for k in range(Ly):
        ...
        inter_interaction_next = np.zeros((Ly, N_orb, Ly, N_orb, Ly, N_orb, Ly, N_orb), dtype=complex)
        for q in range(Ly):
            intra_interaction[:, 0, :, 0, :, 0, :, 0] += \
                V / Ly * cos_k[q] * (n_q[q][:, :, np.newaxis, np.newaxis] * n_q[-q][np.newaxis, np.newaxis, :, :])
            inter_interaction[:, 0, :, 0, :, 0, :, 0] += \
                V / Ly * (1 + np.exp(-1j * 2 * np.pi/Ly * q)) * (n_q[q][:, :, np.newaxis, np.newaxis] * n_q[-q][np.newaxis, np.newaxis, :, :])
            ...
            #inter_interaction[:, 0, :, 0, :, 0, :, 0] += \
            #    V / Ly * (1 + np.exp(-1j * 2 * np.pi/Ly * q) + 1/np.sqrt(3) * np.exp(1j * 2 * np.pi/Ly * q) + 1/np.sqrt(3) * np.exp(- 2 * 1j * 2 * np.pi/Ly * q)) * \
            #(n_q[q][:, :, np.newaxis, np.newaxis] * n_q[-q][np.newaxis, np.newaxis, :, :])

            #inter_interaction_next[:, 0, :, 0, :, 0, :, 0] += \
            #    V / np.sqrt(3) / Ly * np.exp(-1j * 2 * np.pi/Ly * q) * (n_q[q][:, :, np.newaxis, np.newaxis] * n_q[-q][np.newaxis, np.newaxis, :, :])



        #self.add_intra_ring_interaction(intra_interaction)
        #self.add_inter_ring_interaction(inter_interaction, 1)
        #self.add_inter_ring_interaction(inter_interaction_next, 2)
        self.add_intra_ring_interaction(intra_interaction)
        self.add_inter_ring_interaction(inter_interaction, 1)
...
chi_list = {0:400, 20:800, 40:2000}
dmrg_params = {"trunc_params": {"svd_min": 1.e-10}, "mixer": True, "chi_list" : chi_list,"max_sweeps": 100}

sites = M.lat.mps_sites()
#psi = MPS.from_product_state(sites,["full","empty","full","empty","empty","empty"]*3,"infinite")
psi = MPS.from_product_state(sites,["full","empty","empty"]* ((Lx * Ly) // 3),"infinite")

info = dmrg.run(psi, M, dmrg_params)
energy = info['E']
delta_energy = abs(info["sweep_statistics"]['E'][-1] - info["sweep_statistics"]['E'][-2])

data = {'psi': psi, 'dmrg_params': dmrg_params, 'Energy': energy, 'Delta Energy': delta_energy, 'model_params': model_params}

filename = 'pi_third_mixed_6_6/2000_data_spinless_mixed_V_' + str(V_test) + '.pkl'
